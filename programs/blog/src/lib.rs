...
pub mod blog {
    ...
    //Initialize blog functio

    pub fn init_blog(ctx: Context<InitBlog>) -> ProgramResult {
    // get accounts from ctx
        let mut blog_account = &mut ctx.accounts.blog_account;
        let mut genesis_post_account = &mut ctx.accounts.genesis_post_account;
...
pub struct InitBlog<'info> {
    ...
    pub mut blog_account: Account<'info, BlogState>,
...
pub struct SignupUser<'info> {
    ...
    pub mut user_account: Account<'info, UserState>,
    pub authority: Signer<'info>,
    pub system_program: Program<'info, System>,
}
...
#[derive(Accounts)]
    pub struct CreatePost<'info> {
        ...
        pub mut post_account: Account<'info, PostState>,
